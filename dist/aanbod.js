...
(() => {
  ...
  window.Webflow.push(() => {
    ...
    if (consent) {
      ...
      closeConsent?.addEventListener("click", () => {
        gsapWithCSS.to(consent, { opacity: 0, yPercent: 200 });
      });
    }
    ...
    mm.add(
      ...
      (context3) => {
        ...
        if (isDesktop) {
          ...
          if (cardsContainer) {
            const cards = gsapWithCSS.utils.toArray('[cs-el="card"]');
            if (cards.length > 0) {
              cards.forEach((item) => {
                const overlay = item.querySelector('[cs-el="cardContent"]');
                gsapWithCSS.set(overlay, { y: "101%" });
                const icon = item.querySelector('[cs-el="cardIcon"]');
                const tl_hover = gsapWithCSS.timeline({ paused: true });
  ...
  window.Webflow.push(() => {
    ...
    if (modal) {
      gsapWithCSS.set(modal, { autoAlpha: 0 });
...
gsap/Observer.js:
  ...
  *)
*/
//# sourceMappingURL=aanbod.js.map
